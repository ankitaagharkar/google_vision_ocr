...
class Scan_OCR:
    ...
    def get_passport_details(self, image_path):
        try:
            ...
            self.passport_details.put(
                (self.text, passport_number, name, dob, issue_date, expiry_date, result, keys, values))
    ...
    def get_doc_text(self, path, doc_type):
        ...
        self.doc_text.put((self.text, employer_full_address, employer_street, employer_state, employer_zipcode,
                           ...
                           current_gross_pay, ytd_gross_pay, current_net_pay, ytd_net_pay, taxes, current_taxes,
                           ytd_taxes, rate_taxes, hrs_taxes, earnings, current_earnings, ytd_earnings, rate_regular,
                           hrs_regular, pre_deduction, current_pre_deduction, ytd_pre_deduction, rate_pre_deduction,
                           hrs_pre_deduction, post_deduction, current_post_deduction, ytd_post_deduction,
                           rate_post_deduction, hrs_post_deduction, total_calculated_taxes,
                           current_total_calculated_taxes, ytd_total_calculated_taxes, hrs_total_calculated_taxes,
                           rate_total_calculated_taxes, total_calculated_regular, current_total_calculated_regular,
                           ytd_total_calculated_regular, hrs_total_calculated_regular, rate_total_calculated_regular,
                           total_calculated_pre, current_total_calculated_pre, ytd_total_calculated_pre,
                           hrs_total_calculated_pre, rate_total_calculated_pre, total_calculated_post,
                           current_total_calculated_post, ytd_total_calculated_post, hrs_total_calculated_post,
                           rate_total_calculated_post, total_taxes, current_total_taxes, ytd_total_taxes,
                           hrs_total_taxes, rate_total_taxes, total_regular, current_total_regular, ytd_total_regular,
                           hrs_total_regular, rate_total_regular, total_pre, current_total_pre, ytd_total_pre,
                           hrs_total_pre, rate_total_pre, total_post, current_total_post, ytd_total_post,
                           hrs_total_post, rate_total_post, employment_Start_date, pay_date, position,
                           result_output_data, employee_id, emp_start_date))
    ...
    def image_to_pdf(self, image_path, doc_type):
        ...
        try:
            ...
            if 'SSN' in doc_type:
                ...
                f_path = str(self.config["system_pdf_path"]) + filename1
                process = subprocess.call(
                    'convert -density 300 -trim ' + image_path1 + ' -quality 100 -append ' + f_path, shell=True)

            else:
                process = subprocess.call(
                    'convert -density 300 -trim ' + image_path1 + ' -quality 100 -append ' + f_path_png, shell=True)
                f_path = str(self.config["system_pdf_path"]) + filename2
                process = subprocess.call('convert ' + f_path_png + ' -background white -alpha remove ' + f_path,
                                          shell=True)
            print("done")
    ...
    def get_license_confidences(self, data, text, result):
        ...
        self.get_license_confidence.put(
            (dict, date_score, address_score, license_score, other_score, f_name_score, m_name_score, l_name_score,
             data))
    ...
    def get_ssn_pay_location(self, value_json, image, doc_type, result=''):
        ...
        if 'SSN' in doc_type:
            ...
            self.passport_no, self.passport_dict, filename = self.sp_location.get_passport_location(value_json, image,
                                                                                                    result)
    ...
    def confidence_score(self, path, doc_type, data, keys='', values=''):
        try:
            ...
            if 'SSN' in doc_type:
                ...
                passport_no_score, passport_fn_score, passport_mn_score, passport_ln_score, passport_date_score = self.score.passport_confidence(
                    data, keys, values)
                self.confidence.put(
                    (passport_no_score, passport_fn_score, passport_mn_score, passport_ln_score, passport_date_score))

        except Exception as e:
            self.custom_print("Exception in all_details confidence_score fun.", e)

    def get_license_response(self, data, response, conf_result, doc_type, zipcode, text, image_path, result,
                             pdf_image_path):
        ...
        for i in range(len(response['fields'])):
            for j in range(len(actual_value)):
                if response['fields'][i]['name'] == actual_value[j]:
                    ...
                    pass
        if pdf_image_path == '':
            thread = threading.Timer(1, self.get_location,
                                     (data, image_path, doc_type, result))
        else:
            thread = threading.Timer(1, self.get_location,
                                     (data, pdf_image_path, doc_type, result))
        thread.setName("Location Thread")

        thread.start()

        (address_location, licence_id_location, dict_location, file_path1) = self.location.get()
        thread = threading.Timer(1, self.get_license_confidences,
                                 (data, text, conf_result,))
        thread.setName("Confidence Thread")
        thread.start()

        (date_dict, date_score, address_score, license_score, other_score, f_name_score, m_name_score, l_name_score,
         adata) = self.get_license_confidence.get()
        reg = ''
    ...
    def all_details(self, response):
        ...
        try:
            ...
            if 'License' in json_val[doc_id]:
                ...
                thread = threading.Timer(1, self.get_license_all_details,
                                         ("../images/documents_upload/" + filename,))
                print("another request")

                thread.start()
                
                (text_response, self.licence_id, self.expiry, self.dob, self.issue, self.address, self.name, self.state,
                 self.zipcode, self.city, date_val, result, pdf_image_path, flag) = self.get_license_details.get()
                if self.licence_id == ' ' and self.expiry == '' and self.dob == '' and self.issue == '' and self.address == '' and self.name == '' and self.state == '' and self.zipcode == '' and self.city == '' and date_val:
                    self.scan_result[
                        'error_msg'] = "Document upload was NOT successful due to unclear image or unrecognizable image; please [upload document again] or [rescan image].  Note: Please ensure that the full document is visible, and make sure there are no markings on the document that might be blocking any text or numbers."
                    self.scan_result['status'] = "INCORRECT_DOCUMENT"
                if self.name == '':
                    ...
                    self.name_value = self.name.split()
                    if self.state.lower() == 'nj':
                        name_val = self.name.split()
                        if len(name_val) == 2:
                            if re.search(r'\b(!?[A-Za-z]+\s[A-Za-z])\b', self.name):
                    ...
                    if re.search('(!?JR|Jr|jr|jR)', self.name):
                        if self.name_value.index(re.findall(r'(!?JR|Jr|jr|jR)', self.name)[0]) == 1:
                            self.name_value[1] = self.name_value[1] + " " + self.name_value[2]
                            self.name_value.pop(2)
                        elif self.name_value.index(re.findall('(!?JR|Jr|jr|jR)', self.name)[0]) == 3:
                            self.name_value[2] = self.name_value[2] + " " + self.name_value[3]
                            self.name_value.pop(3)
                        elif self.name_value.index(re.findall('(!?JR|Jr|jr|jR)', self.name)[0]) == 2:
                            self.name_value[1] = self.name_value[1] + " " + self.name_value[2]
                            self.name_value.pop(2)
                ...
                for i in range(len(self.state_value['data'])):
                    if self.state_value['data'][i]['state'] == self.state:
                ...
                if len(self.name_value) == 0:
                    data = {'first_name': "", 'dob': self.dob, 'issue_date': self.issue,
                            'expiration_date': self.expiry,
                            'last_name': "", 'address': self.address, 'license_id': self.licence_id,
                            "middle_name": "", "state": self.state, "postal_code": self.zipcode, "city": self.city,
                            "date_val": date_val}
                elif len(self.name_value) == 1:
                    data = {'first_name': "", 'dob': self.dob, 'issue_date': self.issue,
                            'expiration_date': self.expiry,
                            'last_name': self.name_value[0], 'address': self.address,
                            'license_id': self.licence_id,
                            "middle_name": "", "state": self.state, "postal_code": self.zipcode, "city": self.city,
                            "date_val": date_val}
                elif 'FN_MN_LN_SUF' == name_seq:

                    if len(self.name_value) == 3:
                        data = {'first_name': self.name_value[0], 'dob': self.dob,
                                'issue_date': self.issue,
                                'expiration_date': self.expiry,
                                'last_name': self.name_value[2], 'address': self.address,
                                'license_id': self.licence_id,
                                "middle_name": self.name_value[1], "state": self.state,
                                "postal_code": self.zipcode,
                                "city": self.city, "date_val": date_val}
                    elif len(self.name_value) == 4:
                        data = {'first_name': self.name_value[0], 'dob': self.dob,
                                'issue_date': self.issue,
                                'expiration_date': self.expiry,
                                'last_name': self.name_value[2] + " " + self.name_value[3],
                                'address': self.address, 'license_id': self.licence_id,
                                "middle_name": self.name_value[1], "state": self.state,
                                "postal_code": self.zipcode,
                                "city": self.city, "date_val": date_val}
                    elif len(self.name_value) == 5:
                        data = {'first_name': self.name_value[0] + " " + self.name_value[1] + " " +
                                              self.name_value[2], 'dob': self.dob,
                                'issue_date': self.issue,
                                'expiration_date': self.expiry,
                                'last_name': self.name_value[4],
                                'address': self.address, 'license_id': self.licence_id,
                                "middle_name": self.name_value[3], "state": self.state,
                                "postal_code": self.zipcode,
                                "city": self.city, "date_val": date_val}
                    else:
                        data = {'first_name': self.name_value[0], 'dob': self.dob,
                                'issue_date': self.issue,
                                'expiration_date': self.expiry, 'last_name': self.name_value[1],
                                'address': self.address,
                                'license_id': self.licence_id, "middle_name": '', "state": self.state,
                                "postal_code": self.zipcode, "city": self.city, "date_val": date_val}
                elif 'LN_FN_MN_SUF' == name_seq:
                    if len(self.name_value) == 3:
                        data = {'first_name': self.name_value[1], 'dob': self.dob,
                                'issue_date': self.issue, 'expiration_date': self.expiry,
                                'last_name': self.name_value[0], 'address': self.address,
                                'license_id': self.licence_id,
                                "middle_name": self.name_value[2], "state": self.state,
                                "postal_code": self.zipcode, "city": self.city, "date_val": date_val}
                    elif len(self.name_value) == 4:
                        data = {'first_name': self.name_value[1], 'dob': self.dob,
                                'issue_date': self.issue, 'expiration_date':self.expiry,
                                ...
                                "state": self.state,
                                "postal_code": self.zipcode, "city": self.city,
                                "date_val": date_val}
                    elif len(self.name_value) == 5:
                        data = {'first_name': self.name_value[3], 'dob': self.dob,
                                'issue_date': self.issue, 'expiration_date': self.expiry,
                                'last_name': self.name_value[0] + " " + self.name_value[1] + " " +
                                             self.name_value[2], 'address': self.address,
                                'license_id': self.licence_id,
                                "middle_name": self.name_value[4], "state": self.state,
                                "postal_code": self.zipcode, "city": self.city,
                                "date_val": date_val}
                    else:
                        data = {'first_name': self.name_value[1], 'dob': self.dob,
                                'issue_date': self.issue,
                                'expiration_date': self.expiry, 'last_name': self.name_value[0],
                                'address': self.address,
                                'license_id': self.licence_id, "middle_name": '', "state": self.state,
                                "postal_code": self.zipcode, "city": self.city, "date_val": date_val}
                ...
                self.text = text
                thread = threading.Timer(1, self.get_license_response,
                                         (data, response, conf_result, json_val[doc_id], self.zipcode, text,
                                          "../images/documents_upload/" + filename, result, pdf_image_path,))
                ...
                if filename.rsplit('.', 1)[1] == 'pdf':
                    thread = threading.Timer(1, self.image_to_pdf,
                                             ("../images/documents_upload/" + filename,
                                              json_val[doc_id],))
                    thread.start()
                    ...
                    image_path = f_path
                    # image_path=f_path
                    # thread = threading.Timer(1,self.image_processing_threading,
                    #                           (f_path, json_val[doc_id],))
                else:
                ...
                thread = threading.Timer(1, self.get_doc, (image_path, json_val[doc_id],))
                thread.start()
                time.sleep(5)
                ...
                if SSN_Number == '' and name == '' and date == '':
                    ...
                    thread = threading.Timer(1, self.get_ssn_pay_location,
                                             (add, image_path, json_val[doc_id],))
                    thread.start()
                    time.sleep(5)
                    (ssn_number_location, name_location, date_location,
                     file_path1) = self.location.get()

                    thread = threading.Timer(1, self.confidence_score, (
                ...
                if re.search('PDF', filename.rsplit('.', 1)[1], re.IGNORECASE):
                    ...
                    thread = threading.Timer(1, self.get_doc_text,
                                             (f_path, json_val[doc_id],))
                ...
                time.sleep(5)
                image_path = self.image_processing.get()
                thread = threading.Timer(1, self.get_passport_details, (image_path,))
                thread.start()
                (self.text, passport_number, name, dob, issue_date, expiry_date, result, keys,
                 values) = self.passport_details.get()
                if passport_number == '' and name == '' and dob == '' and issue_date == '' and expiry_date == '':
                    ...
                    if len(name.split()) == 3:
                        passport = {'passport_no': passport_number, 'first_name': name.split()[1],
                                    'middle_name': name.split()[2], 'last_name': name.split()[0], 'dob': dob,
                                    'issue_date': issue_date, 'expiration_date': expiry_date}
                    elif len(name.split()) == 2:
                        passport = {'passport_no': passport_number, 'first_name': name.split()[1], 'middle_name': '',
                                    'last_name': name.split()[0], 'dob': dob, 'issue_date': issue_date,
                                    'expiration_date': expiry_date}
                    elif len(name.split()) == 4:
                        passport = {'passport_no': passport_number, 'first_name': name.split()[1],
                                    'middle_name': name.split()[2] + " " + name.split()[3],
                                    'last_name': name.split()[0], 'dob': dob,
                                    'issue_date': issue_date, 'expiration_date': expiry_date}
                    ...
                    thread = threading.Timer(1, self.get_ssn_pay_location,
                                             (passport, image_path, json_val[doc_id], result,))
                    thread.start()
                    time.sleep(5)
                    (passport_no_location, passport_dict_location, file_path1) = self.location.get()

                    thread = threading.Timer(1, self.confidence_score,
                                             (image_path, json_val[doc_id], passport, keys, values,))
                    thread.start()
                    time.sleep(5)
                    (passport_no_score, passport_fn_score, passport_mn_score, passport_ln_score,
                     passport_date_score) = self.confidence.get()
                            ...
                            for key, value in passport_no_location.items():
                                ...
                                if key in response['fields'][i]['field_value_original']:
                                    ...
                                    if self.config['field_level'] == "True":
                                        ...
                                        if self.config['high_accuracy'][0] <= int(passport_no_score) <= \
                                                self.config['high_accuracy'][1]:
                                            response['fields'][i]['confidence'] = passport_no_score

                                        elif self.config['medium_accuracy'][0] <= int(passport_no_score) <= \
                                                self.config['medium_accuracy'][1]:
                                            response['fields'][i]['confidence'] = passport_no_score
                ...
                identification_score = int((detected_value_count / len(response['fields'])) * 100)
                all_confidence_score = int(passport_no_score) + int(passport_fn_score) + int(passport_mn_score) + int(
                    passport_ln_score) + int(passport_date_score)
                document_score = int((all_confidence_score / 3))
