...
class get_all_sp_location:
    ...
    def paystub_value(self, value_data, path,result):
        img = cv2.imread(path)
        value_json={}
        ...
        for key, value in value_data.items():
            if "field_value_original" in value_data[key]:
                if value['field_value_original'] != "" or value['field_value_original'] != None:
                    ...
                    if value[0] != '':
                        ...
                        if "field_value_original" in value_json[key1]:
                                if value[0] in value1['field_value_original']:
                                    ...
                                    elif "deduction" in value1['section_name']:
                                        vrx = np.array(value[1], np.int32)
                                        vrx = vrx.reshape((-1, 1, 2))
                                        img = cv2.polylines(img.copy(), [vrx], True, (0, 255, 255), 1)
                                        var_name = "deduction" + str(c)
                            ...
                            if re.search(r'(?!' + re.escape(value[0]) + r')', value1):
                                ...
                                elif value[0] in value_json['employee_mn']:
                                # print("in else",value[1])
                                    vrx = np.array(value[1], np.int32)
                                    vrx = vrx.reshape((-1, 1, 2))
                                    img = cv2.polylines(img.copy(), [vrx], True, (0, 255, 255), 1)
                                    self.employee_name.update({str(value[0]): value[1]})
                                elif value[0] in value_json['employee_ln']:
                                # print("in else",value[1])
                                    vrx = np.array(value[1], np.int32)
                                    vrx = vrx.reshape((-1, 1, 2))
                                    img = cv2.polylines(img.copy(), [vrx], True, (0, 255, 255), 1)
                                    self.employee_name.update({str(value[0]): value[1]})
                                elif value[0] in value_json['employee_fn']:
                                # print("in else",value[1])
                                    vrx = np.array(value[1], np.int32)
                                    vrx = vrx.reshape((-1, 1, 2))
                                    img = cv2.polylines(img.copy(), [vrx], True, (0, 255, 255), 1)
    ...
    def paystub_get_location(self,data,image,result):
        value_data,value_json={},{}
        for i in range(len(data['fields'])):
            if 'field_value_original' in data['fields'][i]:
                    ...
                    if 'regular' in data['fields'][i]['name']:
                        ...
                        value_data[data['fields'][i]['name'] ] = data['fields'][i]['field_value_original']
                        value_data[data['fields'][i]['name'] + "_ytd"] = data['fields'][i]['optional_value']
                    elif 'employee_mn' in data['fields'][i]['name']:
                        value_data[data['fields'][i]['name']] = data['fields'][i]['field_value_original']
                    elif 'employee_ln' in data['fields'][i]['name']:
                        value_data[data['fields'][i]['name']] = data['fields'][i]['field_value_original']
                    elif 'employee_fn' in data['fields'][i]['name']:
                        value_data[data['fields'][i]['name']] = data['fields'][i]['field_value_original']
        ...
        thread=threading.Thread(target=self.paystub_value,args=(value_data,image,result,))
        thread.start()
