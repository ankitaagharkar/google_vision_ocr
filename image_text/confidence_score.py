...
class text_score:
    ...
    #     try:
    #         result = dict(zip(keys, values))
    #         for value[0], value[1] in result.items():
    #             for key1, value1 in data.items():
    #                 if value[0] != '' and value1 != '':
    #                     if re.search(r'\b(=?' + re.escape(value[0]) + r')\b', value1):
    #                         if value[0] in data['ssn_number']:
    #                             self.ssn_confidence_score = value[1]
    #                         elif value[0] in data['ssn_date']:
    #                             self.ssn_date_confidence_score = self.ssn_date_confidence_score + value[1]
    #                         else:
    #                             self.ssn_name_confidence_score = self.ssn_name_confidence_score + value[1]
    #
    #         self.ssn_score = int((self.ssn_confidence_score * 100))
    #         if self.ssn_score >= 100:
    #             self.ssn_score = 87
    ...
    #         c = 0
    #
    #         for value[0], value[1] in data_val.items():
    #             if "field_value_original" in data_val[value[0]]:
    #                 if value[1]['field_value_original'] != "":
    #                     data.update({value[0]:value[1]})
    #             else:
    #                     data.update({value[0]: value[1]})
    #
    #         # print("in None Data",data)
