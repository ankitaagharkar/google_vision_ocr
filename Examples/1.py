    ...
    def rectify_data(self):
        ...
        for key, values in enumerate(res):
            if values[1][0][1] < prev_y_mid and abs(prev_y_start-values[1][0][1]) <= mod_ht:
                ...
                prev_y_mid = int((prev_y_start + prev_y_end) / 2)
            ...
            for w_index,word in enumerate(line):
                ...
                if x:
                    if x[0] in ['current','this period','current period','current month','is period','this','ratecurrent']:
                        ...
                        if current_flag:
                            current_flag = False
                            current.append([current_mean,current_end,current_y])
                            ...
                            try:
                                ...
                                ytd_end = 0
                            ytd.append([ytd_mean,ytd_end,ytd_y])
                    ...
                    if self.is_float_or_int(word[0]):
                        ...
                        prev_word = word[0]
                        #check if this word is blocking any current or ytd column
                        #if yes, we need to stop that block
                        word_mean = (word[1][0][0] + word[1][1][0])/2
                        ...
                        if abs(mean_diff_c[1][0] - word_mean) < space_width and abs(mean_diff_c[1][2] - word[1][0][1]) > line_height:
                            
                            current[mean_diff_c[0]] = [9999,9999,9999]
                            ytd[mean_diff_c[0]] = [9999,9999,9999]
                        if abs(mean_diff_y[1][0] - word_mean) < space_width and abs(mean_diff_y[1][2] - word[1][0][1]) > line_height:
                            current[mean_diff_y[0]] = [9999,9999,9999]
                            ytd[mean_diff_y[0]] = [9999,9999,9999]
            current_flag = False
        print(blocks)
