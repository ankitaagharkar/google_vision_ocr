...
class get_all_location:
    ...
    def find_column_sequence(self,line,word,probable_list,columns,blocks):
        ...
        for seq in probable_list:
            print(seq)
            seq_word = difflib.get_close_matches(word[0],[s[0] for s in seq],cutoff=0.80)
            ...
            try:
                ...
                for s in range(s_index,len(seq)):
                    ...
                    if difflib.get_close_matches(seq[s][0],[temp_word[0]],cutoff=0.75):
                        match_found = match_found + 1
                        try:
                            if line[temp_w_index+1][1][0][0] > temp_word[1][1][0]:
                                if not difflib.get_close_matches(temp_word[0]+line[temp_w_index+1][0],check_flag,cutoff=0.80):
    ...
    def get_payslip_amounts(self,lines):
        ...
        for line in lines:
            ...
            for w_index,word in enumerate(line):
                ...
                if b:
                    print('\nI am in B',word[0])
                    blocks.append([b[0],word[1][0][0],word[1][0][1],mean_x,abs(word[1][0][1]-word[1][3][1]),True])
                    print(blocks[-1])
                    block_flag = True
                ...
                if c:
                    ...
                    try:
                        seq_word = difflib.get_close_matches(word[0],[j[0] for i in all_headers['col_sequence'] for j in i],cutoff=0.80)
                        if seq_word:
                            print('We have got a start of some sequence(s)',seq_word)
                            probable_list = []
                            for s in seq_word:
                                for i in all_headers['col_sequence']:
                            ...
                            if status:
                                all_headers['col_sequence'].remove(final_seq)
                                continue
                            if status:
                                print('We found sequence',columns)
                            else:
                    ...
                    try:
                        ...
                        if abs(mean_diff[1][3]-mean_x) < mean_width or abs(mean_diff[1][0]-word[1][0][0]) < mean_width:
                            ...
                            columns.pop(mean_diff[0])
                            print('we are blocking some column',columns)
                    except:
                        pass
    ...
    def create_blocks(self,data_blocks):
        ...
        for i,db in enumerate(data_blocks):
            ...
            if db[2] == 'Rate':
                ...
                final_data[block_name].append([db[0],db[1],None,None,None])
                prev_word = db[0]
                ...
                if db[0] == prev_word and block_name == prev_block:
                    ...
                    final_data[block_name].append([db[0],None,db[1],None,None])
        for x in final_data:
                ...
                for dt in final_data[x]:
                    if dt[0] != 'Total':
                        ...
                        try:
                            ...
                            pass
                final_data[x].append(['Total_Calculated',"{:.2f}".format(cal_cur),"{:.2f}".format(cal_ytd),None,None]) 
        return final_data
...
try:
    ...
    paystub_type = 'Generic'
print('we are here',paystub_type)
...
for table in paystub_structure.findall('./'):
    if table.tag != 'col_sequence':
        ...
        all_headers['col_sequence'] = []
        for val_type in table.findall('./'):
            all_headers['col_sequence'].append([])
            for vals in val_type.findall('./'):
                try:
                    all_headers['col_sequence'][-1].append([vals.text,vals.attrib])
                except:
                    all_headers['col_sequence'][-1].append([vals.text,None])

x = get_all_location(all_headers)
...
for line in lines:
    print(line)
data_blocks = x.get_payslip_amounts(lines)
final_data = x.create_blocks(data_blocks)
print(final_data)