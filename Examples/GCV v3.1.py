...
class get_all_location:
    def __init__(self):
        ...
        self.deduction_headers = ['deductions','eductions','deductions statutory','statutory deductions','deductions & credits','deductions from gross',
                                'before tax','before tax deductions','pre tax','pre tax deductions','pre-tax deductions','before-tax deductions',
                                'after tax','after tax deductions','after-tax deductions','post tax deductions','post-tax deductions']
    ...
    def get_payslip_amounts(self,lines):
        block_header_flags = self.earning_headers + self.deduction_headers + self.tax_headers + ['information','others','memo information',
                                'summary',
                                'colleague','employer',
                                'net pay distribution','direct deposit distribution','direct deposit information','direct deposit',
            ...
            for w_index,word in enumerate(line):
                ...
                if c:
                    ...
                    try:
                        ...
                        end_x = 0
                    columns.append([word[1][0][0],word[1][0][1],end_x,mean_x,None,None,None,abs(word[1][0][1]-word[1][3][1])])
                    ...
                    try:
                        ...
                        mean_diff = min(y_diff,key=lambda x: mean_x - x[1][3] if mean_x - x[1][3] >= 0 else float('inf'))
                        b_index = blocks.index(mean_diff[1])
        ...
        for i,db in enumerate(data_blocks):
            ...
            if x:
                ...
                if db[3].lower() in self.earning_headers:
                    ...
                    block_name = 'Others'
            if db[2] == 'Current':
                final_data[block_name].append([db[0],db[1],None])
                prev_word = db[0]
                ...
                if db[0] == prev_word and block_name == prev_block:
                    ...
                    final_data[block_name].append([db[0],None,db[1]])
        return final_data
...
x.get_text(sys.argv[1])
point1 = timer()
print('Time for GCV is ',point1 - start)
...
print('Time for searching Data blocks is ',end - point1)
