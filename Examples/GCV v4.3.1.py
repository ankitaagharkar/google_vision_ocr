...
class get_all_location:
    ...
    def create_blocks(self,data_blocks):
        ...
        other_columns = ['checking ','net check','savings','direct deposit','checkng','Chck1','dir dip check','your federal taxable wages this period']
        ...
        for i,db in enumerate(data_blocks):
            ...
            pc = difflib.get_close_matches(db[0].lower(),net_columns+gross_columns+other_columns,cutoff=0.80)           
            if pc:
                ...
                bn = difflib.get_close_matches(db[3].lower(),self.earning_headers+self.deduction_headers+self.tax_headers,cutoff=0.80)
                if bn:
            ...
            if db[3].lower() in ['summary_table','summary'] and block_name == 'Others':
                summary_word = difflib.get_close_matches(db[0].lower(),[s[0].lower() for s in self.data_dict['summary_table']],cutoff=0.85)
                if summary_word:
            ...
            if db[2] == 'Rate':
                ...
                if db[0] == prev_word and block_name == prev_block:
                    ...
                    prev_word = ''
                    prev_block = ''
                elif db[0] == prev_word and is_star_pre and star_found:
...
def get_structure(root,all_headers,paystub_type):
...
all_headers = {}
all_headers = get_structure(root,all_headers,'Generic')
...
try:
    ...
    for st in paystub_structs:
        if st in f_name:
            print(st)
            all_headers = get_structure(root,all_headers,st)
            break
except:
    pass
#initialise class with all the headers that are to be used for paytub-data reading
