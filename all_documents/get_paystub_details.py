...
class Paystub_details:
    def __init__(self):
        ...
        self.pay_begin=["pay period start","pay begin date","pay period begin","begin date","period beginning","period beginning date","period starting",
                        "period Start",]
    ...
    def paystub_address(self,block,emp_id):
        ...
        try:
            ...
            if len(block["addresses"])==[]:
                ...
                code = data[0][0]
                code1 = data1[0][0]
                street = re.findall(r'(!?(ONE|TWO|THREE|FOUR|FIVE|SIX|SEVEN|EIGHT|NINE|TEN)\-?\.?\w?(\w+)?\s?\&?\-?\s?(\w+)?\s?[A-Za-z]+|\d+\-?\.?\w?\d+\s?\-?\s?\d+\s?[A-Za-z]+\s?([A-Za-z]+)?\s?([A-Za-z]+)?|\w?\d+\s?\-?\s?(\d+)?\s?[A-Za-z]+)', address1)[0][0]
                street1 = re.findall(r'(!?(ONE|TWO|THREE|FOUR|FIVE|SIX|SEVEN|EIGHT|NINE|TEN)\-?\.?\w?(\w+)?\s?\&?\-?\s?(\w+)?\s?[A-Za-z]+|\d+\-?\.?\w?\d+\s?\-?\s?\d+\s?[A-Za-z]+\s?([A-Za-z]+)?\s?([A-Za-z]+)?|\w?\d+\s?\-?\s?(\d+)?\s?[A-Za-z]+)', address2)[0][0]
