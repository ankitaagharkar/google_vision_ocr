...
class Paystub_details:
    def __init__(self):
        ...
        self.pay_begin = ["pay period start", "pay begin date", "pay period begin", "begin date", "period beginning",
                          ...
                          'earns begin date', 'check stub for the period','pay begin dato','begin at',"period begin"]
        ...
        self.pay_all = ["pay period from", "period beg/end", "pay period", "payroll period", "period date", 'period',
                        'begin/end dates','for pay period','pay perlod','payperiod','period']
    ...
    def paystub_address(self, block, emp_id, employer_name, employee_name):
        ...
        try:
            ...
            if len(block["addresses"]) == []:
                ...
                if "1100 SAN LEANDRO BLVD SUITE #400".rstrip().lstrip().lower() in block["addresses"][0]['address'][0].rstrip().lstrip().lower():
                    block['addresses'].pop(0)
                    print("pop address",block['addresses'])
                if re.search(
                    ...
                    if len(block["addresses"][0]['address']) > 1:
                        ...
                        if not re.search(r'(!?ONE|THREE|FOUR|FIVE|SIX|SEVEN|EIGHT|NINE|TEN)',address1[0].split()[0], re.IGNORECASE) and not re.search('([A-Za-z]+\s?\d+)?|\d+', address1[0].split()[0], re.IGNORECASE):
                ...
                if re.search('\d'+emp_id+'\d+',address2) or re.search(r'\d'+emp_id,address2) or re.search(emp_id+'\d+',address2):
                    pass
                else:
                    address2 = address2.replace(emp_id, '')

                if re.search('\d' + emp_id + '\d+', address1) or re.search(r'\d' + emp_id, address1) or re.search(emp_id + '\d+', address1):
                    pass
                else:
                ...
                with open("../config/address_post_code", "r") as post_val:
                ...
                addresses1 = self.c.find_between_r(address1, street, code).replace("  "," ")
                addresses2 = self.c.find_between_r(address2, street1, code1).replace("  "," ")
                ...
                with open("../config/address_post_code", "r") as post_val:
                    post_code = post_val.read()
                if re.search(r'(\s|\,)?\b(!?' + state_name + ')(\s|\.|\,|\-|\,\s)+(' + post_code + r')\b', full_address,re.IGNORECASE):
                ...
                if re.search(r'(\s|\,)?\b(!?' + state_name + ')(\s|\.|\,|\-|\,\s)+(' + post_code + r')\b', full_address1,re.IGNORECASE):
                ...
                if actual_city2 == '':
                    ...
                    city2 = actual_city2
                if len(re.findall(r'new\syork|new\sjersey', full_address.lower())) == 2:
                    city1 = re.findall(r'new\syork|new\sjersey', full_address.lower())[0]
                if len(re.findall(r'new\syork|new\sjersey', full_address1.lower())) == 2:
                    city2 = re.findall(r'new\syork|new\sjersey', full_address1.lower())[0]
            ...
            if state.lower() == 'york' or state.lower() == 'jersey':
                state = 'New' + " " + state
            if state1.lower() == 'york' or state1.lower() == 'jersey':
    ...
    def get_paystub_date(self, block):
        ...
        try:
                        ...
                        if x:
                            if x[0] in self.pay_frequency:
                                ...
                                if re.search(r'\d+', pay_freq):
                                    ...
                                    elif re.search(r'\s(thru|through)\s', pay_freq, re.IGNORECASE):
                                        pay_freq = pay_freq.split(re.findall(r'\s(!?thru|through)\s', pay_freq)[0])
                                    elif re.search(r'(thru|through)\s', pay_freq, re.IGNORECASE):
                                        pay_freq = pay_freq.split(re.findall(r'(!?thru|through)\s', pay_freq)[0])
                                    elif re.search(r'\s(thru|through)', pay_freq, re.IGNORECASE):
                                        pay_freq = pay_freq.split(re.findall(r'\s(!?thru|through)', pay_freq)[0])
                                    elif re.search(r'(thru|through)', pay_freq, re.IGNORECASE):
                                        pay_freq = pay_freq.split(re.findall(r'(!?thru|through)', pay_freq)[0])
                                ...
                                elif re.search(r'\s(thru|through)\s', paya, re.IGNORECASE):
                                    paya = paya.split(re.findall(r'\s(!?thru|through)\s', paya)[0])
                                    start_date = paya[0]
                                    end_date = paya[1]
                                elif re.search(r'(thru|through)\s', paya, re.IGNORECASE):
                                    paya = pay_freq.split(re.findall(r'(!?thru|through)\s', paya)[0])
                                    start_date = paya[0]
                                    end_date = paya[1]
                                elif re.search(r'\s(thru|through)', paya, re.IGNORECASE):
                                    paya = pay_freq.split(re.findall(r'\s(!?thru|through)', paya)[0])
                                    start_date = paya[0]
                                    end_date = paya[1]
                                elif re.search(r'(thru|through)', paya, re.IGNORECASE):
                                    paya = paya.split(re.findall(r'(!?thru|through)', paya)[0])
                                    start_date = paya[0]
                                    end_date = paya[1]
            ...
            if len(datev) == 3:
                st = parse(re.findall(r'((0[0-9]|1[0-2])\s?[./-]([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[./-](19|20|21|22)\d\d|(0[0-9]|1[0-2])[./-]([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-]\d\d|([0-9]|0[0-9]|1[0-2])[./-](0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](19|20|21|22)\d\d|([0-9]|0[0-9]|1[0-9])\s?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](19|20|21|22)\d\d|([0-9]|0[0-9]|1[0-9])[./-](0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?(19|20|21|22)\d\d|([0-9]|[1-9]|0[0-9]|1[0-2])\s?[./-]?([0-9]|[1-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[./-]?(19|20|21|22)\d\d|(0[0-9]|1[0-2])\s?[,:./-]?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[,:./-]?(19|20|21|22)\d\d|(0\s?[0-9]|1\s?[0-2])\s?[,:./-]\s?([0-9]|0\s?[1-9]|1\s?[0-9]|2\s?[0-9]|3\s?[0-1])\s?[,:./-]?(19|20|21|22)\d\d|([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[,:./-]?\s?(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)\,?\s?(19|20|21|22)\d\d|(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)\s?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[,:./-]?\,?\s?(19|20|21|22)\d\d|(19|20|21|22)\d\d\s?[,:./-]?(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[,:./-]?([0-9]|[1-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])|([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[./-](19|20|21|22)\d\d|(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[./-]?\s([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-]\,?\s?(19|20|21|22)\d\d)',start_date)[0][0])
                st = st.strftime('%m/%d/%Y')
                et = parse(re.findall(r'((0[0-9]|1[0-2])\s?[./-]([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[./-](19|20|21|22)\d\d|(0[0-9]|1[0-2])[./-]([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-]\d\d|([0-9]|0[0-9]|1[0-2])[./-](0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](19|20|21|22)\d\d|([0-9]|0[0-9]|1[0-9])\s?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](19|20|21|22)\d\d|([0-9]|0[0-9]|1[0-9])[./-](0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?(19|20|21|22)\d\d|([0-9]|[1-9]|0[0-9]|1[0-2])\s?[./-]?([0-9]|[1-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[./-]?(19|20|21|22)\d\d|(0[0-9]|1[0-2])\s?[,:./-]?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[,:./-]?(19|20|21|22)\d\d|(0\s?[0-9]|1\s?[0-2])\s?[,:./-]\s?([0-9]|0\s?[1-9]|1\s?[0-9]|2\s?[0-9]|3\s?[0-1])\s?[,:./-]?(19|20|21|22)\d\d|([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[,:./-]?\s?(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)\,?\s?(19|20|21|22)\d\d|(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)\s?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[,:./-]?\,?\s?(19|20|21|22)\d\d|(19|20|21|22)\d\d\s?[,:./-]?(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[,:./-]?([0-9]|[1-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])|([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[./-](19|20|21|22)\d\d|(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[./-]?\s([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-]\,?\s?(19|20|21|22)\d\d)',end_date)[0][0])
                et = et.strftime('%m/%d/%Y')
                pd = parse(re.findall(r'((0[0-9]|1[0-2])\s?[./-]([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[./-](19|20|21|22)\d\d|(0[0-9]|1[0-2])[./-]([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-]\d\d|([0-9]|0[0-9]|1[0-2])[./-](0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](19|20|21|22)\d\d|([0-9]|0[0-9]|1[0-9])\s?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](19|20|21|22)\d\d|([0-9]|0[0-9]|1[0-9])[./-](0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?(19|20|21|22)\d\d|([0-9]|[1-9]|0[0-9]|1[0-2])\s?[./-]?([0-9]|[1-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[./-]?(19|20|21|22)\d\d|(0[0-9]|1[0-2])\s?[,:./-]?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[,:./-]?(19|20|21|22)\d\d|(0\s?[0-9]|1\s?[0-2])\s?[,:./-]\s?([0-9]|0\s?[1-9]|1\s?[0-9]|2\s?[0-9]|3\s?[0-1])\s?[,:./-]?(19|20|21|22)\d\d|([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[,:./-]?\s?(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)\,?\s?(19|20|21|22)\d\d|(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)\s?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[,:./-]?\,?\s?(19|20|21|22)\d\d|(19|20|21|22)\d\d\s?[,:./-]?(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[,:./-]?([0-9]|[1-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])|([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[./-](19|20|21|22)\d\d|(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[./-]?\s([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-]\,?\s?(19|20|21|22)\d\d)',pay_date)[0][0])
                ...
                if start_date != '':
                    sd = parse(re.findall(r'((0[0-9]|1[0-2])\s?[./-]([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[./-](19|20|21|22)\d\d|(0[0-9]|1[0-2])[./-]([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-]\d\d|([0-9]|0[0-9]|1[0-2])[./-](0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](19|20|21|22)\d\d|([0-9]|0[0-9]|1[0-9])\s?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](19|20|21|22)\d\d|([0-9]|0[0-9]|1[0-9])[./-](0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?(19|20|21|22)\d\d|([0-9]|[1-9]|0[0-9]|1[0-2])\s?[./-]?([0-9]|[1-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[./-]?(19|20|21|22)\d\d|(0[0-9]|1[0-2])\s?[,:./-]?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])\s?[,:./-]?(19|20|21|22)\d\d|(0\s?[0-9]|1\s?[0-2])\s?[,:./-]\s?([0-9]|0\s?[1-9]|1\s?[0-9]|2\s?[0-9]|3\s?[0-1])\s?[,:./-]?(19|20|21|22)\d\d|([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[,:./-]?\s?(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)\,?\s?(19|20|21|22)\d\d|(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)\s?([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[,:./-]?\,?\s?(19|20|21|22)\d\d|(19|20|21|22)\d\d\s?[,:./-]?(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[,:./-]?([0-9]|[1-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])|([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-](Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[./-](19|20|21|22)\d\d|(Jan|Feb|Mar|Apr|May|Jun|July|Jul|Aug|Sept|Sep|Oct|Nov|Dec|january|february|march|april|may|june|july|august|september|october|november|december)[./-]?\s([0-9]|0[1-9]|1[0-9]|2[0-9]|3[0-1])[./-]\,?\s?(19|20|21|22)\d\d)',start_date)[0][0])
        ...
        try:
            ...
            if employer_name != '' and employer_name != ' ':
                ...
                x = difflib.get_close_matches(employer_name.split()[0].lower(),
                                              [vt.lower() for vt in self.name_list['names']],
                                              cutoff=0.93)
                if 2 >= len(employer_name.rstrip().lstrip().split()) < 4:
                    ...
                    y = difflib.get_close_matches(employer_name.split()[0].lower(),
                                                  [vt.lower() for vt in self.surname_list['surnames']],
                                                  cutoff=0.93)
                elif len(employer_name.rstrip().lstrip().split())>=4:
                    y = difflib.get_close_matches(employer_name.split()[3].lower(),
                                                  [vt.lower() for vt in self.surname_list['surnames']],
                                                  cutoff=0.93)
                        ...
                        for i in range(len(text_value["addresses"])):
                            if text_value["addresses"][i]['name'] != [] :
                                ...
                                if text_value["addresses"][i]['name'][0].replace('.','').lower() == employee_name.lower():
                                    ...
                                    if re.search(
                                            r'(\s|\,)\b(!?' + state_name + ')(\s|\.|\,|\-|\s?\-\s?)+(' + post_code + r')\b',
                                    ...
                                    if code=='ON':
                                        code=" "+code
                                    addresses1 = self.c.find_between_r(temp_emp_address, street, code).replace('  ','')
                        ...
                        for i in range(len(text_value["addresses"])):
                            if text_value["addresses"][i]['name'] != []:
                                ...
                                if text_value["addresses"][i]['name'][0].replace('.','').lower() == employer_name.lower():
                                    ...
                                    addresses2 = self.c.find_between_r(temp_employer_address, street1, code1).replace('  ','')
                    ...
                    for i in range(len(gross_net_values)):
                        if 'Others' in pays_keys[i]:
                            for j in text_value['Others']:
                                    ...
                                    if not x1:
                                        ...
                                        for k in range(len(text_value["addresses"])):
                                            ...
                                            if employee_name.replace(',', '').replace('.', '').rstrip().lower() in text_value["addresses"][k]['name'][0].replace(',', '').replace('.','').rstrip().lower():
                                                if employee_address.rstrip().lstrip().lower() in text_value["addresses"][k]['address'][0].rstrip().lstrip().lower():
                                                    break
                                                else:
                                                    ...
                                                    with open("../config/states", "r") as all_state_val:
                                                        state_name = all_state_val.read().replace('\n', '')
                                                    with open("../config/address_post_code",
                                                              "r") as post_val:
                                                        post_code = post_val.read()
                                                    ...
                                                    if re.search(
                                                        ...
                                                        data = re.findall(r'(\s|\,)\b(!?' + state_name + ')',
                                                                          temp_emp_address)
                                                        if len(data) > 1:
                                                            if re.search(r'(!?York|Jersey)', data[0][1], re.IGNORECASE):
                                                                data[0] = ('', '')
                                                    ...
                                                    if code == 'ON':
                                                        code = " ON"
                                                    addresses1 = self.c.find_between_r(temp_emp_address, street,
                                                                                       code).replace('  ', '')
                                                    ...
                                                    if re.search(
                                                        ...
                                                        state, zipcode, city = self.c.get_address_zipcode(full_address,
                                                                                                          code)
                                                    else:
                                                        state, zipcode, city = self.c.get_address_zipcode(full_address,
                                                                                                          code)
                                                    ...
                                                    if actual_city1 == '':
                                                        city = ' '.join(
                                                            map(str, full_address.split(code, 1)[0].split()[-1:]))
                                                        city = city.lower()
                                                    else:
                                                        city = actual_city1
                                                    ...
                                                    actual_full_address = self.c.find_between_r(full_address.lower(),
                                                                                                street.lower(),
                                                                                                city)
                                                    actual_full_address = street.lower() + actual_full_address
                                ...
                                break
            if re.search('(\s|\,)(!?Inc|LLC|CORP|LLP|LPS)', employee_name, re.IGNORECASE):
                en = employee_name
                ed = employee_address
                es = employee_street
                ez = employee_zipcode
            ...
            if '19001 CRESCENT SPRINGS DRIVE'.lower() in employer_address.lower():
                employer_address=employee_address
                employer_state=employee_state
                employer_street=employee_street
                employer_city=employee_city
            ...
            elif '19001 CRESCENT SPRINGS DRIVE'.lower() in employee_address.lower():
                    employee_address = ''
                    employee_state = ''
                    employee_street = ''
                    employee_city = ''
            ...
            employee_name=employee_name.replace('Ø','O')
