        ...
        if lookup_found:
            for pt in paystub_types:
                if difflib.get_close_matches(lookup_found[0].lower(),[x.lower() for x in paystub_types[pt]],cutoff=0.90):
                    ...
                    return pt
        self.custom_print('No type found, using Generic')
        return 'Generic'
...
class paystub_address:
    ...
    def get_state_coordinates(self,description,result):
        ...
        state_name = re.findall(r'\b(!?AL|AK|AS|AZ|AŽ|AŻ|AR|CA|CO|CT|DE|DC|FM|FL|GA|GU|HI|ID|IL|IN|IA|KS|KY|LA|ME|MH|MD|MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|MP|OH|OK|OR|PW|PA|PR|RI|SC|SD|TN|TX|UT|VT|VI|VA|WA|WV|WI|WY)\,?\s(\d{5}(?:\s?\-\s?\d{1,4})|\d{5}(?:\s?\.\s?\d{4})|\d{5})', str(self.description))
        state = state_name
        all_points = []
        if not state:
            return False, []
        ...
        for _, values in enumerate(self.result):
            if state_found:
                if values[0] == state[len(all_points)][1]:
    ...
    def extract_address(self,data_box,state_name):
        ...
        junk_words = ['employee address','employee name','address','employer address','employer name']
        for db in reversed(data_box):
            print('values are ',db[0])
...
def main():
    ...
    if ext == 'pdf':
        ...
        subprocess.call('convert -density 300 -trim ' +image_path+ ' -quality 100 '+out_path,shell=True)
