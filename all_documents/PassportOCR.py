...
class Passport_Details:
    ...
    def get_passport_no(self,text):
        ...
        try:
            passport_regex=re.findall(r'(!?\d{9}|[A-Za-z]{2,3}\d{5,6}|[A-Za-z]{1,2}\d{7,8}|[A-Za-z]{1,2}\d{6,7})\s',text)
    ...
    def get_passport_name(self,text,passport_number,date_regex):
        ...
        try:
            ...
            name=" ".join(map(str,re.findall('\s[A-Z]{2,}',text_split[1])[:5]))
            ...
            if name=='':
                name_regex=" ".join(map(str,re.findall('[A-Z]+<<[A-Z]+<[A-Z]+|[A-Z]+<<[A-Z]+',text))).replace('<',' ')
                name=name_regex.replace(re.findall(r'[A-Z]{3}',name_regex)[0],'',1).replace('  ',' ')
            else:
                name_regex = " ".join(map(str, re.findall('[A-Z]+<<[A-Z]+<[A-Z]+<[A-Z]+|[A-Z]+<<[A-Z]+<[A-Z]+|[A-Z]+<<[A-Z]+', text))).replace('<',' ')
                temp_name = name_regex.replace(re.findall(r'[A-Z]{3}', name_regex)[0], '', 1).replace('  ', ' ')
    ...
    def get_all_dates(self,text):
        ...
        try:
            ...
            print(e)
            actual_dob_date=actual_issue_date=actual_expiry_date=''
            return actual_dob_date,actual_issue_date,actual_expiry_date
