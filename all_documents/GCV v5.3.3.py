...
class paystub_gcv:
            ...
            if self.pay_components[-1][3] == l_index:
                ...
                if not self.pay_components[-1][2]:
                    ...
                    try:
                        ...
                        #self.custom_print('Comparing pay components ',word[0],min_diff_current,min_diff_prev)
    ...
    def rectify_data(self,pa_points,mode_slant,mode_height):
        ...
        for _, values in enumerate(res):
            ...
            if values[1][0][1] < prev_y_mid and abs(prev_y_start-values[1][0][1]) <= mod_ht:
                ...
                """
                if round((values[1][0][1]+values[1][3][1])/2) < prev_y_mid and abs(prev_y_start-values[1][0][1]) <= mod_ht:
                    line_list[-1].append([values[0],values[1]])
                """
            else:
            ...
            for w_index,word in enumerate(line):
                ...
                if abs(word_end - word[1][0][0]) <= space_width and not new_line_start:
                    ...
                    if d1 in desc:
                        ...
                        col_word = difflib.get_close_matches(d1.lower(),self.column_header_flags+['period start date','period end date'],cutoff=0.85)
                        if col_word:
                            try:
                                #if its next word is not a part of this entire word
                    ...
                    #if word belongs to column header, no further word should be added in it.
                    col_word = difflib.get_close_matches(word[0].lower(),self.column_header_flags,cutoff=0.85)
                    if general_space_width < (abs(word[1][0][1] - word[1][2][1]) * height_wise_multiplier):
                        space_width = general_space_width
                    else:
                        space_width = abs(word[1][0][1] - word[1][2][1]) * height_wise_multiplier
                    ...
                    if col_word:
                        #if its next word is not a part of this entire word
                        try:
                            if not difflib.get_close_matches(word[0].lower()+' '+line[w_index+1][0].lower(),self.column_header_flags+['hours and','earnings and'],cutoff=0.85):
                                self.custom_print('we are here making space width zero ',word)
                                space_width = 0
                        except:
                            self.custom_print('we are here making space width zero by except ',word)
                            space_width = 0
                    word_end = word[1][2][0]
                    word_val = word[0]
                    prev_index = w_index
        ...
        for l_index,line in enumerate(lines):
            ...
            for w_index,word in enumerate(line): 
                ...
                if b:
                    ...
                    if block_flag:
                        self.custom_print('It might block someone')
                            ...
                            # closest block' mean and its respective 'Current' type column's start-x
                                ...
                                for i,x in enumerate(columns):
                                    if x[4] == b_index and x[6] == 'Current' and blocks[-1][3] > blocks[b_index][3] and blocks[-1][3] < x[0]:
                                        change_block = True
                            if change_block:
                                self.custom_print('we are changing blocks')
                ...
                if c:
                    ...
                    try:
                        ...
                        if num_cols > 1:
                            self.custom_print('We might have to delete some column')
                            ...
                            try:
                                ...
                                self.custom_print('candidate ',mean_diff)
                        ...
                        #pass
                    self.custom_print(columns,blocks)
                    ...
                    if val:
                        ...
                        for i, e in reversed(list(enumerate(columns))):
                            if word[1][1][0] > columns[i][3] and (word[1][1][0] <= columns[i][2] or columns[i][2] == 0):
                                ...
                                if l_index > self.pay_components[-1][3]:
                                    try:
    ...
    def create_blocks(self,data_blocks):
        net_columns = ['net pay','total net','net earnings','net wages']
    ...
    def paystub_details(self,path):
        ...
        if paystub_type == 'ADP':
            gp, np = self.check_gross_net(final_data)
            if not gp or not np:
                ...
                self.values=[]
                self.description = []
                text_output_data, hl, sl, rl = self.get_text(path,clear_image)
                ...
                gp_new, np_new = self.check_gross_net(new_final_data)
...
class paystub_address:
    ...
    def extract_address(self,data_box,state_name):
        ...
        junk_words = ['employee address','employee name','address','employer address','employer name','order','to the','of']
