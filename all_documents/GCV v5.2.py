...
class paystub_gcv:
    ...
    def rectify_data(self,pa_points,max_height):
        state_lines = []
        line_list = []
        line_heights = []
        ...
        for key, values in enumerate(res):
            ...
            if abs(values[1][0][1] - values[1][3][1]) > max_height:
                continue
                    ...
                    if val:
                        ...
                        for i, e in reversed(list(enumerate(columns))):
                            if word[1][1][0] > columns[i][3] and (word[1][1][0] <= columns[i][2] or columns[i][2] == 0):
                                ...
                                if prev_word == prev_component and prev_word not in ['Total','TOTAL']:
                                    if identified_block:
    ...
    def check_gross_net(self,final_data):
        try:
            if float(final_data['Gross Pay'][0][1]) != float(final_data['Earnings'][-1][1]):
                return False
            if float(final_data['Net Pay'][0][1]) > float(final_data['Gross Pay'][0][1]):
                return False
            return True
        except:
            return False
    ...
    def paystub_details(self,path):
        ...
        print('Slant Mode ',mode_slant)
        mode_rotate = max(set(rl), key=rl.count)
        print('Rotate Mode ',mode_rotate)
        ...
        #determine max height based on mode height of words
        mode_height = max(set(hl), key=hl.count)
        max_height = mode_height * 2
        ...
        lines,state_lines = self.rectify_data(pa_points,max_height)
        ...
        if paystub_type == 'ADP':
            if not self.check_gross_net(final_data):
